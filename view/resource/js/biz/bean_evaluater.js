(function($, window) {
	...
	$("#create_user_submit").click(function(e) {
		...
		$.ajax({
			url : requestContextPath + "/evaluater/addoa.html",
			type : "POST",
			data : {
	...
	function showRecord(id, $record) {
		...
		Util.ajax({
			url : requestContextPath + "/evaluater/getUserOpRecord.html",
			data : {
				"id" : id
			},
			dataType : "json",
	...
	$jTable = $table.table({
		ajaxUrl : requestContextPath + "/evaluater/userList.html",
		paramsJson : getParam(),
		pageKey : {
			total : "total",
			pageSize : "pageSize",
	...
	window.exportData = function() {
		...
		Util.ajax({
			url : requestContextPath + "/evaluater/exportUserData.html",
			data : getParam(),
			cache : false,
			dataType : "text",
			success : function(data) {
			...
			.on("click", '.editrow',
					function(e) {
						...
						if (!$thatTr.find(".arrow").length) {
							...
							selectedCompany('edit_companyId', '评估机构', curData.companyId);
	...
	$table.on("click", '#user_detail_btnSave', function(e) {
		 ...
		 */
		$.ajax({
			url : requestContextPath + "/evaluater/editoa.html",
			type : "POST",
			data : {
				'id' : id,
	...
	$table.on("click", '#edit_reset_password', function(e) {
		...
		$.ajax({
			url : requestContextPath + "/evaluater/resetUserPassword.html",
			data : {
				'ids' : ids,
			},
			dataType : "json",
	...
	$table.on("click", '#edit_unlock', function(e) {
		...
		$.ajax({
			url : requestContextPath + "/evaluater/unLockUser.html",
			data : {
				'englishName' : englishname,
			},
			dataType : "json",
