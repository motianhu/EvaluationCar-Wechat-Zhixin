~function($, window) {
	...
	var getUserParam = (function() {
		...
		return function() {
			_queryParam.category = $('#carApplyUserClass').val() || '高级评估师';
					...
					function() {
						...
						Util
								.ajax(
										{
											url : requestContextPath
													+ "/carBill/editApplyZjCarBill.html",
											type : "POST",
											data : {
												'carBillId' : $(
														"#hidden_carBillId")
	...
	$("#data_cancel").click(function() {
		...
		// 先校验
		Util.ajax({
			url : requestContextPath + "/carBill/cancelAndEditZjBill.html",
			type : "POST",
			data : {
				'carBillId' : $("#hidden_carBillId").val()
			},
