~function($, window) {
		...
		tbodyMaker : function(data) {
			...
			if (items) {
				for (var i = 0, len = items.length; i < len; i++) {
					...
					list.push("<span rowid='" + curRow.carBillId + "'>" + curRow.carBillId + "</span>"); //编号
	...
	function showDetail() {
		...
		if (tr == null) {
			...
			//先校验
			$.ajax({
				url : requestContextPath + "/carBill/getAndEditCsBill.html",
				type : "POST",
				data : {
					'carBillId' : carBillId,
				},
				...
				success : function(data) {
					if (!data.success) {
						...
						$("#images").width(document.body.scrollWidth - 10);
						$("#images").height(document.body.scrollHeight - 50);
						var $editFrame = $("#displayimagesframe");
						$editFrame.attr("src", requestContextPath + "/view/admin/canvas/cs_carbilldetail_ext.jsp?carBillId=" + carBillId);
						$editFrame.attr("width", document.body.scrollWidth - 10);
						$editFrame.attr("height", document.body.scrollHeight - 50);
					}
				},
