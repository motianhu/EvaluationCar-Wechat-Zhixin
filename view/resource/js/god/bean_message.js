~function($, window) {
	...
	function addDict() {
		...
		var isAllUser = $('#add_isAllUser').prop("checked") ? "-1" : "";

		receiveUser = getIdAutoComplete(receiveUser);

		$.ajax({
			type : "POST",
			url : requestContextPath + "/messagemanager/add.html",
			data : {
				title : title || '',
				content : content || '',
	...
	window.exportData = function() {
		...
		Util.ajax({
			url : requestContextPath + "/messagemanager/export.html",
			data : getParam(),
			cache : false,
			dataType : "json",
			success : function(data) {
				// location.href = data;
				$.fileDownload(data.object);
			}
		}, "正在导出，请稍等...");
	}
	 ...
	 */
	$jTable = $table.table({
		ajaxUrl : requestContextPath + "/messagemanager/getList.html",
		paramsJson : getParam(),
		pageKey : {
			total : "total",
			pageSize : "pageSize",
	...
	$table.on("click", '#dict_detail_confirm', function(e) {
		...
		$.ajax({
			type : "POST",
			url : requestContextPath + "/messagemanager/edit.html",
			data : {
				id : id,
