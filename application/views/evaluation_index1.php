...
if( isWeiXin()){
	window.location.href = "<?php echo base_url().'index.php/evaluation/index/'.$id.'/0/'.$isEdit;?>";
}
</script>
...
<body>
	<div style="display:none">
		<form id="upload_form" enctype="multipart/form-data" method="post" action="evaluation.php">
		<input type="file" id="file" name="image_file" />
		</form>
	</div>
	<form id="pg_form" name="pg_form" action="<?php echo base_url().'index.php/evaluation/do_eval/1';?>" method="post">
	<div class="MainBody pg">
		<div class="mainInfoBody">
			<div style="float:left;"><a href="<?php echo base_url().'index.php/audit';?>">审核查询</a></div>
			<div style="float:right;">
				<span></span>您好:<?php echo $chineseName.'('.$englishName.')';?></span>
				<a href="<?php echo base_url().'index.php/login/login_out';?>">退出</a>
			</div>
			<!------------------登记证--------------------->
			<header class="header">
		      <section>
...
var images = {
	...
	function up(t){
		...
		var url = "<?php echo base_url().'index.php/evaluation1/up_image/';?>"+i_type+'/'+i_no;
	...
	$('#file').change(function(){
		...
		$.ajax({
			...
			success:function(data){
				if(data.status == '1'){
					t.css({"background-image":"url("+data.src+")","background-size":"100% 100%"});
				}else{
					alert(data.msg);
