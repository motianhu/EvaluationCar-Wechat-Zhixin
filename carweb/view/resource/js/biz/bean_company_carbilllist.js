~function($, window) {
	 ...
	 */
	$jTable = $table.table({
		ajaxUrl : requestContextPath + "/carBill/getCompanyAllBillList.html",
		paramsJson : getParam(),
		pageKey : {
			total : "total",
			pageSize : "pageSize",
		...
		tbodyMaker : function(data) {
			...
			if (items) {
				for (var i = 0, len = items.length; i < len; i++) {
					...
					list.push(curRow.csStartTime);
					list.push(curRow.cjStartTime);
					list.push(curRow.zjStartTime);
					list.push(curRow.gjStartTime);
					list.push(curRow.csMinutesStr);
					list.push(curRow.cjMinutesStr);
					list.push(curRow.zjMinutesStr);
					list.push(curRow.gjMinutesStr);
					list.push(curRow.allMinutesStr);
					list.push(curRow.fallBackCount);
