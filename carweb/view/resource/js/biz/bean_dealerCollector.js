(function($, window) {
	...
	$("#create_user_submit").click(function(e) {
		...
		$.ajax({
			url : requestContextPath + "/dealerCollector/addoa.html",
			type : "POST",
			data : {
	...
	function showRecord(id, $record) {
		...
		Util.ajax({
			url : requestContextPath + "/dealerCollector/getUserOpRecord.html",
			data : {
				"id" : id
			},
			dataType : "json",
	...
	$jTable = $table.table({
		ajaxUrl : requestContextPath + "/dealerCollector/userList.html",
		paramsJson : getParam(),
		pageKey : {
			total : "total",
			pageSize : "pageSize",
	...
	window.exportData = function() {
		...
		Util.ajax({
			url : requestContextPath + "/dealerCollector/exportUserData.html",
			data : getParam(),
			cache : false,
			dataType : "text",
			success : function(data) {
						...
						if (!$thatTr.find(".arrow").length) {
							...
							var temp = rowTemplateHtml.replace(/{rowid}/g,
									...
									.replace("{email}", email || '').replace(
											...
											"{edit_workExperience}",
											curData.workExperience || '')
									.replace("{edit_brithDay}",
											curData.brithDay || '').replace(
											"{edit_workTime}",
											curData.workTime || '').replace(
											"{edit_telphone}",
											curData.telphone || '').replace(
											"{edit_homePhone}",
											curData.homePhone || '').replace(
											"{edit_speciality}",
											curData.speciality || '').replace(
											"{edit_education}",
											curData.education || '').replace(
											"{edit_graduateSchool}",
											curData.graduateSchool || '')
									.replace("{edit_major}",
											curData.major || '').replace(
											"{edit_address}",
											curData.address || '');
							...
							$("#edit_category").find(
									"option[value=" + curData.category + "]")
									.attr("selected", true);
							$("#edit_regoin").find(
									"option[value=" + curData.regoin + "]")
									.attr("selected", true);
							$("#edit_sex").find(
									"option[value=" + curData.sex + "]").attr(
									"selected", true);
							...
							selectedUserCompany('edit_companyId',
									curData.companyId);
	...
	$table.on("click", '#user_detail_btnSave', function(e) {
		 ...
		 */
		$.ajax({
			url : requestContextPath + "/dealerCollector/editoa.html",
			type : "POST",
			data : {
				'id' : id,
	...
	$table.on("click", '#edit_reset_password', function(e) {
		...
		$.ajax({
			url : requestContextPath
					+ "/dealerCollector/resetUserPassword.html",
			data : {
				'ids' : ids,
			},
			dataType : "json",
	...
	$table.on("click", '#edit_unlock', function(e) {
		...
		$.ajax({
			url : requestContextPath + "/dealerCollector/unLockUser.html",
			data : {
				'englishName' : englishname,
			},
			dataType : "json",
